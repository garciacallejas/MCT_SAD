...
pred.subset <- pred.subset %>% group_by(year.predicted,plot,metric) %>% 
...
pred.wide.delta <- pred.wide[,c("year.predicted","plot","metric","intensity","delta.fd","delta.nd","delta.ia","delta.id")]
names(pred.wide.delta) <- c("year","plot","metric","intensity","fd","nd","ia","id")
...
ggsave(filename = "results/images/observed_predicted.pdf",
       plot = obs.pred.plot,
       device = cairo_pdf,
       width = 6,height = 4,dpi = 300)

ggsave(filename = "results/images/predicted_perturbations.pdf",
       plot = pred.dif.plot,
       device = cairo_pdf,
