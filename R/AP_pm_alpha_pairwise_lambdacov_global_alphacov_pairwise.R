...
#' @export
AP_pm_alpha_pairwise_lambdacov_global_alphacov_pairwise <- function(par,
                                                                    fitness,
  ...
  for(z in 1:ncol(all_neigh_matrix)){
    interactions_term <- interactions_term - (alpha[z] + alpha_cov_term[[z]]) * log((my.g[z]*all_neigh_matrix[,z]) + 1) * all_neigh_matrix[,z] 
  }
