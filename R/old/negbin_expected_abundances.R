...
lambda.negbin <- read.csv2(file = "../Spatial_coexistence/results/lambda_negbin_multilevel_heterogeneous_both_clean.csv",header = TRUE,stringsAsFactors = FALSE)
alpha.negbin <- read.csv2(file = "../Spatial_coexistence/results/alpha_negbin_multilevel_heterogeneous_both_clean.csv",header = TRUE,stringsAsFactors = FALSE)
  ...
  for(i.plot in 1:length(plots)){
    for(i.sub in 1:length(subplots)){
      ...
      if(length(present.sp)>1){
        ...
        my.lambda <- lambda.negbin[lambda.negbin$year == years[i.year] &
                                            lambda.negbin$plot == plots[i.plot] &
                                            lambda.negbin$sp %in% present.sp,c("sp","lambda")]
        sp.lambda <- my.lambda$lambda[match(present.sp,my.lambda$sp)]
        names(sp.lambda) <- present.sp
        
        # build alpha matrix
        my.alpha <- alpha.negbin[alpha.negbin$year == years[i.year] &
                                       alpha.negbin$plot == plots[i.plot] &
                                       alpha.negbin$focal %in% present.sp &
                                       alpha.negbin$neighbour %in% present.sp,]
        
        sp.alpha <- matrix(my.alpha$magnitude,
                           byrow = TRUE,
                           nrow = length(present.sp),
                           ncol = length(present.sp),
                           dimnames = list(present.sp,present.sp))
...
# write.csv2(projected.abund,file = "./results/projected_abundances_subplot_negbin.csv",row.names = FALSE)
